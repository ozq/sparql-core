 ...
 */
var WSparql = function () {
    /**
     * @param configuration
     */
    function WSparql(configuration) {
        _classCallCheck(this, WSparql);
        ...
        this.methods = [{
            name: 'wearing',
            toSparql: function toSparql(triple, postfix) {
                var tripleParts = this.sparqlFormatter.getTripleParts(triple);
                var relationPart = '';
            ...
            toWsparql: function toWsparql(query) {
                var _this = this;
                ...
                var queryLines = _.compact(query.split('\n'));
                var optionalPartRegexp = new RegExp('\s*optional', 'i');
                var relationFunctionRegexpTemplate = '^\\s*relation\\((' + this.sparqlFormatter.uriRegexpCode + ')\\s+' + '(.*' + '[#:]%predicate_postfix%' + ')' + '\\s' + this.sparqlFormatter.uriRegexpCode + '\\)$';
                var relationFunctionRegexpCode = '\\s*relation\\((' + this.sparqlFormatter.uriRegexpCode + ')\\s+' + this.sparqlFormatter.uriRegexpCode + '\\s' + this.sparqlFormatter.uriRegexpCode + '\\)';
                var objectRelationRegexpCode = '\\s*requiredRelation\\((' + this.sparqlFormatter.uriRegexpCode + ')\\s+' + '(.*' + '[#:]object' + ')' + '\\s' + '(' + this.sparqlFormatter.uriRegexpCode + ')' + '\\)';
                var wrgTripleRegexp = new RegExp(this.sparqlFormatter.wrgTripleRegexpCode, 'i');
                ...
                var _loop = function _loop(i) {
                    ...
                    if (optionalPartRegexp.test(line)) {
                        ...
                        if (wrgTripleRegexp.test(line)) {
                            var tripleParts = _this.sparqlFormatter.getTripleParts(line);
                            lastFilledItem = {
                                triple: _.trim(line, '. '),
                            ...
                            if (requiredObjectRelationMatch) {
                                ...
                                var wearingDataItem = _.find(wearingData, function (data) {
                                    return data.object === relationSubject;
                                });
                                if (wearingDataItem) {
                                ...
                                _.forEach(['object', 'date_from', 'date_to'], function (postfix) {
                                    ...
                                    if (relationFunctionMatch) {
                                        var _relationSubject = relationFunctionMatch[1];
                                        var _wearingDataItem = _.find(wearingData, function (data) {
                                            return data.object === _relationSubject;
                                        });
                                        if (_wearingDataItem) {
                                            _wearingDataItem.records[postfix] = _.trim(line, '. ');
                                            _wearingDataItem.lines.push(i);
                                            lastFilledItem = _wearingDataItem;
                                        }
                                        relationFunctionFound = true;
                                    }
                                });
                                if (relationFunctionFound === false) {
                                    ...
                                    if (relationFunctionMatch) {
                                        var _relationSubject2 = relationFunctionMatch[1];
                                        var _wearingDataItem2 = _.find(wearingData, function (data) {
                                            return data.postfixData && data.postfixData.object === _relationSubject2;
                                        });
                                        if (_wearingDataItem2) {
                                            _wearingDataItem2.postfixData.triple = _.trim(line, '. ');
                                            _wearingDataItem2.postfixData.hasSiblings = hasSiblings;
                                            _wearingDataItem2.lines.push(i);
                                            lastFilledItem = _wearingDataItem2;
                                        }
                                        relationFunctionFound = true;
                                    }
                                }
            ...
            toSparql: function toSparql(triple) {
                var labelDepth = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;

                labelDepth = _.toInteger(labelDepth);
                var tripleParts = this.sparqlFormatter.getTripleParts(triple);
                var labelPlaceholder = '%labelPlaceholder%';
                var subject = this.filterVariable(tripleParts[2]);
                var objectLabel = this.toLabelVariable(subject);
            ...
            toSparql: function toSparql(triple) {
                ...
                var requiredRelationMethod = _.find(this.methods, function (method) {
                    return method.name === 'requiredRelation';
                });

                var requiredRelationResult = requiredRelationMethod.toSparql.call(this, triple, labelDepth);
            ...
            toWsparql: function toWsparql(query) {
                ...
                for (var i = 0; i < lineCount; i++) {
                    var _line = _.trim(lines[i], '. ');
                    if (whereClauseRegexp.test(_line)) {
                        whereClauseFound = true;
                    } else {
                        if (whereClauseFound === true) {
                            if (self.sparqlFormatter.isTripleLine(_line)) {
                                (function () {
                                    var tripleParts = self.sparqlFormatter.getTripleParts(_line);
                                    if (self.sparqlFormatter.isLabelUri(tripleParts[1])) {
                                        ...
                                        var tripleDataItem = _.find(tripleData, function (data) {
                                            return data.object === tripleParts[0];
                                        });
                                        if (tripleDataItem) {
                                            var tripleBySubject = _.find(tripleData, function (data) {
                                                return data.subject === tripleParts[0];
                                            });
                                            ...
                                            var _tripleDataItem = _.find(tripleData, function (data) {
                                                return _.isEmpty(data.labels) === false && _.last(data.labels).object === tripleParts[0];
                                            });
                                            if (_tripleDataItem) {
                                                _tripleDataItem.labels.push(labelObject);
                                            }
                                        }
                                    } else {
                                        tripleData.push({
                                ...
                                if (optionalPartRegexp.test(_line)) {
                ...
                var requiredRelationMethod = _.find(self.methods, function (method) {
                    return method.name === 'requiredRelation';
                });

                query = self.simplifyRelations(requiredRelationMethod.toWsparql(self.sparqlFormatter.replaceEmptyOperators(lines.join('\n'))).query);
    ...
    _createClass(WSparql, [{
        ...
        value: function loadConfigurationItem(option, defaultValue, type) {
            if (this.configuration.hasOwnProperty(option) && this.configuration[option]) {
                if (typeof type !== 'undefined') {
                    if (_typeof(this.configuration[option]) !== type) {
        ...
        key: 'simplifyRelations',
        value: function simplifyRelations(query) {
            var self = this;
            ...
            var excessOptionalRegexp = new RegExp('OPTIONAL\\s*{\\s*OPTIONAL\\s*{\\s*(' + self.sparqlFormatter.tripleLineRegexpCode + ')\\s*}\\s*}', 'gmi');
        ...
        key: 'filterVariable',
        value: function filterVariable(variable) {
            return _.trim(variable, '-');
        }
    }, {
        key: 'isMinusedVariable',
         ...
         * @returns {boolean}
         */
        value: function isMinusedVariable(variable) {
         ...
         * @returns {boolean}
         */
        value: function isWearingUri(uri) {
        ...
        key: 'getWearingClassByUri',
        value: function getWearingClassByUri(uri) {
        ...
        key: 'filterTriple',
        value: function filterTriple(triple) {
            var self = this;
        ...
        key: 'getMinusedVariables',


        /**
         * @param query
         */
        value: function getMinusedVariables(query) {
        ...
        key: 'toLabelVariable',
        value: function toLabelVariable(variable) {
        ...
        key: 'toSparql',
        value: function toSparql(query) {
            var addSingletonProperties = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
            ...
            _.forEach(self.methods, function (method) {
                ...
                var regexp = new RegExp(method.name + '\\((.*)\\)\\.*', 'gi');
                if (regexp.test(query)) {
                    query = query.replace(regexp, function (match, methodArguments) {
                        var result = method.toSparql.apply(self, methodArguments.split(','));
                        methodWhereVariablesGroup.push(result.whereVariables);
                        return addSingletonProperties === true ? self.sparqlFormatter.addSingletonProperties(result.query, false, '_ws') : result.query;
                    });
                }
        ...
        key: 'toWSparql',
        value: function toWSparql(query) {
            var self = this;
            var methods = self.methods.slice().reverse();

            _.forEach(methods, function (method) {
                if (method.toWsparql) {
                    var methodResult = method.toWsparql.apply(self, [query]);
                    query = methodResult.query;
                }
            });

            var selectVariablesMatch = this.sparqlFormatter.getSelectVariables(query);
            if (selectVariablesMatch) {
                ...
                bodyVariables = bodyVariables.map(function (variable) {
                    return _.trim(variable, '.');
                });
