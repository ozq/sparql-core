...
var SparqlFormatter = function () {
     ...
     * @param options
     */
    function SparqlFormatter(options) {
        _classCallCheck(this, SparqlFormatter);
    ...
    _createClass(SparqlFormatter, [{
        key: 'beautify',
        value: function beautify(content) {
            return this.correctBrackets(this.removeExcessLinesInOperators(this.removeExcessLines(this.removeIndents(content))));
        }
        ...
        key: 'correctBrackets',
        value: function correctBrackets(query) {
            var queryLines = query.split('\n');
            var indentDepth = 0;
            var formattedQueryLines = [];
        ...
        key: 'removeIndents',
        value: function removeIndents(query) {
        ...
        key: 'buildStringWithIndents',
        value: function buildStringWithIndents(string, indentDepth) {
        ...
        key: 'removeExcessLines',
        value: function removeExcessLines(query) {
        ...
        key: 'isUriLikeUrl',
        value: function isUriLikeUrl(uri) {
        ...
        key: 'getClassByUri',
        value: function getClassByUri(uri) {
        ...
        key: 'removeExcessLinesInOperators',
        value: function removeExcessLinesInOperators(query) {
            var queryLines = query.split('\n');
            var indentDepth = 0;
            var formattedContent = [];
            var emptyLineRegexp = new RegExp('^\\s+$', 'gm');
            var operatorContainsRegexp = new RegExp('\\s*\\w+\\s*{', 'gm');
            ...
            for (var i = 0; i < queryLines.length; i++) {
                ...
                if (indentDepth >= 0) {
                    var isCurrentLineEmpty = queryLine.replace(/^\s+|\s+$/g, '') === '' || emptyLineRegexp.test(queryLine);
                    var nextQueryLine = queryLines[i + 1];
        ...
        key: 'removeAllOperatorsByName',
        value: function removeAllOperatorsByName(query, operator) {
                ...
                if (operatorStartIndex >= 0) {
                    ...
                    while (matchedBracket = bracketsRegexp.exec(query)) {
                        ...
                        if (bracketsCounter === 0) {
                            ...
                            var operatorContent = query.substring(operatorStartIndex, bracketsRegexp.lastIndex);
        ...
        key: 'expandUri',
        value: function expandUri(query) {
            ...
            Object.keys(prefixes).map(function (prefix) {
                ...
                var expandedQuery = query.replace(new RegExp(prefix + ':(\\w+)', 'gi'), function (match, property) {
                    return '<' + uri + property + '>';
                });
        ...
        key: 'compactUri',
        value: function compactUri(query) {
            var prefixes = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
            var isStrict = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;

            prefixes = _.merge(this.additionalPrefixes, prefixes);
            Object.keys(prefixes).map(function (prefix) {
                var compactedQuery = null;
                var uri = prefixes[prefix];
                var regExp = isStrict === true ? new RegExp('\<' + uri + '(\\w+)\>', 'gi') : new RegExp(uri + '(\\w+)', 'gi');
        ...
        key: 'removeSingletonProperties',
        value: function removeSingletonProperties(query) {
        ...
        key: 'addSingletonProperties',
        value: function addSingletonProperties(query) {
            var rebuild = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
            var spPostfix = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
            ...
            var replaceStartPosition = query.search(this.getOperatorRegexp('where'));

            var result = query.replace(tripleLineRegexp, function (triple, offset) {
        ...
        key: 'getOperatorRegexp',
        value: function getOperatorRegexp(operator) {
        ...
        key: 'getSelectVariables',
        value: function getSelectVariables(query) {
            var result = query.match(new RegExp('^\\s*select(.*)(\\s+where)', 'mi'));
            if (result) {
        ...
        key: 'getUndefinedVariables',
        value: function getUndefinedVariables(query) {
            var queryLines = query.split('\n');
            ...
            for (var i = 0; i < queryLines.length; i++) {
                var currentString = queryLines[i];
        ...
        key: 'getAllTriples',
        value: function getAllTriples(query) {
            var self = this;
            var triples = query.match(new RegExp(this.tripleLineRegexpCode, 'gmi'));
            if (triples) {
        ...
        key: 'isTripleLine',
        value: function isTripleLine(line) {
        ...
        key: 'getGroupedTripleData',
        value: function getGroupedTripleData(query) {
            var self = this;
            var groupedTripleData = {
                'subjects': [],
                'predicates': [],
                'objects': []
            };

            var triples = this.getAllTriples(query);
            if (triples) {
                triples.forEach(function (triple) {
                    var tripleParts = self.getTripleParts(triple);
                    if (_.includes(groupedTripleData.subjects, tripleParts[0])) {
                        groupedTripleData.subjects.push(tripleParts[0]);
                    }
                    if (_.includes(groupedTripleData.predicates, tripleParts[0])) {
                        groupedTripleData.predicates.push(tripleParts[1]);
                    }
                    if (_.includes(groupedTripleData.objects, tripleParts[0])) {
                        groupedTripleData.objects.push(tripleParts[2]);
                    }
                });
            }
        ...
        key: 'replaceEmptyOperators',
        value: function replaceEmptyOperators(query) {
            var cleanedQuery = query.replace(new RegExp('\\s*\\w+\\s*{\\s*}', 'gmi'), function () {
                return '';
            });
        ...
        key: 'getFirstVariable',
        value: function getFirstVariable(line) {
        ...
        key: 'removeExcessWhitespaces',
        value: function removeExcessWhitespaces(query) {
        ...
        key: 'getTripleParts',
        value: function getTripleParts(triple) {
        ...
        key: 'isRdfTypeUri',
        value: function isRdfTypeUri(value) {
        ...
        key: 'isLabelUri',
        value: function isLabelUri(value) {
        ...
        key: 'buildPredicatesChain',
        value: function buildPredicatesChain(query, subject) {
            var predicatesChain = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
            var rootClass = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;
            ...
            var firstFoundedLine = _.trim(query.match(new RegExp('^\\s*\\?' + subject + '.*', 'mi'))[0]);
            ...
            if (this.isVariable(linePredicate)) {
                ...
                var match = getObjectBySubjectRegexp.exec(query);
        ...
        key: 'buildQueryByPredicatesChain',
        value: function buildQueryByPredicatesChain(chainData) {
            var query = '';
    ...
    return SparqlFormatter;
}();

exports.default = SparqlFormatter;
